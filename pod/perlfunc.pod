...
L<C<Exporter>|Exporter> module.  See L<Exporter>.

If no L<C<import>|/import LIST> method can be found, then the call is
skipped, even if there is an AUTOLOAD method.  This behaviour tends to
hide bugs, so calling a non-existent C<import> method with arguments
has been deprecated since Perl 5.28.  In the present version of Perl
it warns by default.  Starting in Perl 5.32 it will signal an error.
Calling a non-existent C<import> with no arguments is not deprecated.

If you do not want to call the package's L<C<import>|/import LIST>
method (for instance,
to stop your namespace from being altered, or if you know the package
has nothing to export), explicitly supply the empty list:
...
does with VERSION or an omitted or empty LIST
or no C<unimport> method being found, with one exception.  The exception
is that it is deprecated to call a non-existent C<unimport> method even
